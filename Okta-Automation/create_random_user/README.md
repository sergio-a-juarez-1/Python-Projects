...
The script dynamically fetches a standard 10,000-word vocabulary list from MIT's public servers, randomizes first and last names, maps matching safe placeholder email addresses, generates cryptographically secure 20-character passwords, and provisions active accounts via the Okta Users API.

## Features

* **Automated Wordlist Fetching:** Automatically pulls 10,000 common words to form random user combinations.
* **Instant Activation:** Auto-activates provisioned users on creation using native Python boolean serialization (`activate=True`).
* **Cryptographically Secure Passwords:** Replaces deterministic pseudo-random generators with Python's standard `secrets` module to ensure high-entropy, unpredictable password generation.
* **Safe Email Placeholders:** Utilizes `@example.com` fallback routing to protect corporate testing suites from public, anonymous inbox tools like Mailinator.
